  ...
  return (
    <View style={styles.root}>
      <View style={styles.filterCard}>
        ...
        />
        {state.filterErrors.from ? (
          <Text style={styles.error}>{state.filterErrors.from}</Text>
        ) : null}
        ...
        />
        {state.filterErrors.to ? (
          <Text style={styles.error}>{state.filterErrors.to}</Text>
        ) : null}
        {state.filterErrors.range ? (
          <Text style={styles.error}>{state.filterErrors.range}</Text>
        ) : null}
      ...
      {state.monthlySummary ? (
        <View style={styles.summaryCard}>
          <Text style={styles.sectionTitle}>
            Summary {state.monthlySummary.year}-
            {String(state.monthlySummary.month).padStart(2, "0")}
          </Text>
          <Text style={styles.status}>
            Total Amount: {state.monthlySummary.total.toFixed(2)}
          </Text>
          <Text style={styles.status}>
            Transaction Count: {state.monthlySummary.transactionCount}
          </Text>
        </View>
      ) : null}
      ...
      <View style={styles.createCard}>
        ...
        />
        {state.createErrors.quantity ? (
          <Text style={styles.error}>{state.createErrors.quantity}</Text>
        ) : null}
        ...
        />
        {state.createErrors.amount ? (
          <Text style={styles.error}>{state.createErrors.amount}</Text>
        ) : null}
        ...
        <TextInput
          ...
          onChangeText={(value) =>
            setState(model.setCreateTransactionDate(value))
          }
        />
        ...
        />
        {state.createErrors.notes ? (
          <Text style={styles.error}>{state.createErrors.notes}</Text>
        ) : null}
            ...
            renderItem={({ item }) => (
              <TransactionRow
                ...
                onEditQuantity={(value) =>
                  setState(model.setEditQuantity(value))
                }
                onEditAmount={(value) => setState(model.setEditAmount(value))}
                onEditTransactionDate={(value) =>
                  setState(model.setEditTransactionDate(value))
                }
                ...
                onDelete={() => {
                  Alert.alert(
                    "Delete Transaction",
                    `Delete ${item.dailyItemName} on ${item.transactionDate}?`,
                    [
                      { text: "Cancel", style: "cancel" },
                      {
                        ...
                        onPress: () => {
                          void (async () => {
                            try {
                              const next = await model.deleteTransaction(
                                item.id,
                              );
                              setState(next);
                            } catch {
  ...
  if (isEditing) {
    return (
      <View style={styles.row}>
        ...
        />
        {editErrors.quantity ? (
          <Text style={styles.error}>{editErrors.quantity}</Text>
        ) : null}
        ...
        />
        {editErrors.amount ? (
          <Text style={styles.error}>{editErrors.amount}</Text>
        ) : null}
        ...
        {editErrors.transactionDate ? (
          <Text style={styles.error}>{editErrors.transactionDate}</Text>
        ) : null}
        ...
        />
        {editErrors.notes ? (
          <Text style={styles.error}>{editErrors.notes}</Text>
        ) : null}
        {editErrors.form ? (
